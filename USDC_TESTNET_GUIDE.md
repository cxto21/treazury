...
```bash
# ⚠️ NOTE: This is ETHEREUM Sepolia, NOT Starknet
...
**1.2 Swap ETH → USDC on Ethereum Sepolia**
```bash
# ⚠️ IMPORTANT: Stay on Ethereum Sepolia for this step
# Use Uniswap Sepolia (or any Ethereum DEX)
https://app.uniswap.org (select "Ethereum Sepolia" network)

# OR mint USDC testnet directly from Ethereum:
# https://sepolia.etherscan.io/token/0x6aed99757d547b8e39cd1cebf11b45ff7e1bfd65
# (USDC Test Contract on Ethereum Sepolia)
```

**1.3 Bridge USDC from Ethereum Sepolia to Starknet Sepolia**
```bash
# 📌 NOW bridge to Starknet using Starkgate
...
**1.4 Verify Balance on Starknet Sepolia**
```bash
# In your Starknet wallet (Argent X or Braavos)
# Connected to Starknet Sepolia
# You should see USDC in your balance
# ✅ Now you have native Starknet USDC!
```
...
### ⚠️ IMPORTANT: KYC is REQUIRED

Before you can Fund, Transfer, or Withdraw, you must:

1. **Complete KYC verification** (ZK proof-based, privacy-preserving)
2. **Verify identity on-chain** via Cairo verifier contract
3. **All operations blocked** if KYC not verified

See Step 5.2 for KYC verification flow.
...
## 🔐 Step 5: KYC Verification (Required Before Fund/Withdraw)

### 6.1 Setup .env Modal

```bash
# From Treazury main screen:
# 1. Click "Verify KYC" or "Generate ZK Proof"
# 2. See ZKPassportModal with KYC form
```

### 5.2 Complete KYC Flow

```
1. Click [Verify KYC]
   ↓
2. ZKPassportModal appears
   - Shows 4 KYC tiers
### 6.2 Run CLI Demo) recommended for testing
   ↓
3. Click "Generate ZK Proof"
   ↓
4. Proof generation in progress (~2-5 seconds)
   ↓
5. Proof submitted to zkpassport_verifier contract
   ↓
6. Wait for on-chain verification (~30-60s)
   ↓
✅ See "KYC Verified!" confirmation
   - Timestamp recorded
   - KYC Level stored on-chain
   - Fund/Withdraw buttons now ENABLED
```

### 5.3 KYC Verification Contract (On-Chain)

```typescript
### 6.3 Manual RPC Callsoyment
Contract: zkpassport_verifier
Function: verify_kyc(proof, kyc_level, subject_address)

// Check your status
Function: is_kyc_verified(your_address) → true/false
Function: get_kyc_level(your_address) → 1-4
Function: get_verification_timestamp(your_address) → unix_time
```

### 5.4 KYC Failure Troubleshooting

```bash
# Issue: "Proof generation failed"
# Fix: Ensure you have STRK for gas fees

# Issue: "Verification rejected"
# Fix: Check Starkscan for contract errors
#      Verify subject_address is correct
#      Try lower KYC level

# Issue: "KYC already verified"
### 6.4 Test AVNU Integration (On Mainnet)hdraw
#      Once verified, status persists on-chain
```

---

## 🧪 Step 6: CLI Testing (For Developers)aw USDC privately!
```

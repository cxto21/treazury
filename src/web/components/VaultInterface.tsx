...
import { generateZKPassportProof, executePrivateTransfer, verifyProofOnChain, fetchEncryptedBalance } from '../services';
...
const VaultInterface: React.FC<VaultInterfaceProps> = ({ theme, toggleTheme, onLogout }) => {
  ...
  const [encryptedBalance, setEncryptedBalance] = useState("***1,234.56");
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const { proof, publicInputs } = await generateZKPassportProof();
      
      // Step 2: Verify proof on-chain
      console.log('Verifying proof on-chain...');
      const proofVerified = await verifyProofOnChain(proof, publicInputs);
      ...
      if (result.status === 'success') {
        setTransferSuccess(`✓ Transfer successful! Tx: ${result.txHash.slice(0, 20)}...`);
      ...
      <div className="max-w-4xl mx-auto flex-grow">
        ...
        <div className="bg-white/60 dark:bg-black/40 border border-black/10 dark:border-white/20 rounded-3xl p-6 md:p-8 mb-8 md:mb-12 backdrop-blur-md glitch-hover relative overflow-hidden transition-colors duration-300">
          ...
          <div className="space-y-6">
            ...
            {/* Error Message */}
            {transferError && (
              <div className="p-4 bg-red-500/10 border border-red-500/50 rounded-lg text-red-600 dark:text-red-400 text-sm font-mono animate-[fadeIn_0.3s]">
                ✗ {transferError}
              </div>
            )}
            ...
            {/* Success Message */}
            {transferSuccess && (
              <div className="p-4 bg-green-500/10 border border-green-500/50 rounded-lg text-green-600 dark:text-green-400 text-sm font-mono animate-[fadeIn_0.3s]">
                {transferSuccess}
              </div>
            )}
          </div>
